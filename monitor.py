...
def device_command():
    while True:
        ...
        if device is None:
            ...
            # No extend command?
            if c == 'start':
                name = 'Lecture Capture {0} {1}'.format(capture_location, time_now())
                log.info('Name={0}'.format(name))
                resp = device.capture_new_capture(60*90, 'Trinity Standard Lecture', name)
                #print str(resp)
            elif c == 'pause':
                resp = device.capture_pause()
            elif c == 'extend':
                resp = device.capture_extend(600) # 10 minutes
            elif c == 'resume':
